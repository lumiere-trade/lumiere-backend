...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('draft', 'active', 'archived')", name="valid_document_status"
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        op.f("ix_legal_documents_status"), "legal_documents", ["status"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(), nullable=False),
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint("LENGTH(wallet_address) >= 32", name="valid_wallet_length"),
        sa.CheckConstraint("escrow_balance >= 0", name="positive_escrow_balance"),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint("plan_type IN ('free', 'basic', 'pro')", name="valid_plan"),
        sa.CheckConstraint(
            "status IN ('active', 'cancelled', 'expired')", name="valid_status"
        ),
        sa.CheckConstraint(
            "expires_at IS NULL OR expires_at > started_at", name="expires_after_start"
        ),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('pending', 'confirmed', 'failed')", name="valid_tx_status"
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "status IN ('pending', 'completed', 'failed')", name="valid_payment_status"
        ),
        sa.CheckConstraint("amount > 0", name="positive_amount"),
    )
    op.create_index(
        op.f("ix_payments_tx_signature"), "payments", ["tx_signature"], unique=True
    )
    op.create_index(op.f("ix_payments_user_id"), "payments", ["user_id"], unique=False)
    op.create_index(
        "ix_payments_subscription", "payments", ["subscription_id"], unique=False
    )
